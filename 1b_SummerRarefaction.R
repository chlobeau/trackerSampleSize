...
outputFolder <- "C:/Rarefaction/outputs/output_20220605"
...
for(r in 1:length(projects)){
  ...
  seasonalBBs_fldr = paste(projFolders, projName, "UDsSummer", sep = "/")
  UDs_fldr = paste(projFolders, projName, "UDs_popSummer", sep = "/")
  ...
  pp <- foreach(s = 1:nrow(sim_matrix), .combine = rbind,
                .packages = c("raster", "stringr", "spatialEco")) %dopar% {
                  ...
                  for(n in 1:nrow(simulation.df)) {
                    ...
                    simulation.df$allMigs[n] <- NA #na for summer range
  ...
  for(i in 1:nrow(individual.df)){
    ...
    individual.df$newAnimalMigs[i] <- NA #na for summer range
  }
  ...
  ## add column to designate Summer Range
  rare_merged$UD <- "summer"
  
  ## SAVE
  save(rare_merged, file = paste0(outputFolder, "/", "rarefSR_", projName, "_", currentdate,".rda"))
  write.csv(rare_merged, file = paste0(outputFolder, "/", "rarefSR_", projName, "_", currentdate,".csv"), row.names = F)
  save.image(file = paste0(outputFolder, "/", "rarefSR_", projName, "_", currentdate,".RData"))
}
