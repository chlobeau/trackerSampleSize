...
seqs_fldr = paste0(NewProjectFolder, "/sequencesSummer")
metadata_fldr=paste0(NewProjectFolder, "/metadata")

BBs_fldr = paste0(NewProjectFolder, "/UDsSummer")
pop_BBs_out_fldr = paste0(NewProjectFolder, "/UDs_popSummer")
...
#build a database of the potential SUMMER periods

ids <- unique(d$id)
dts <- do.call(rbind, lapply(1:length(ids), function(i){
  yr_rng <- sort(unique(d$year[d$id == ids[i]]))
  tmp2 <- do.call(rbind, lapply(1:length(yr_rng), function(e){
    summer.start <- fl$End.Date[fl$id==ids[i] & fl$seas == "sp" & fl$yr == yr_rng[e]]
    summer.end <- fl$Start.Date[fl$id==ids[i] & fl$seas == "fa" & fl$yr == yr_rng[e]]
    if(length(summer.end)==0){
      summer.end <- NA
      }
    if(length(summer.start)==0){
      summer.start <- NA
      }
    return(data.frame(id=ids[i], summr=paste(yr_rng[e]),
                      id_yr=paste(ids[i],substr(yr_rng[e],3,4),sep="_"),
                      summer.start=as.character(summer.start+86400),
                      summer.end=as.character(summer.end-86400))) 
    }))
  return(tmp2)
  }))

dts$summer.start <- as.character(dts$summer.start) #fix the dates  (it is OK to specify GMT, since all dates will be in GMT!)
dts$summer.end <- as.character(dts$summer.end)

mnths <- as.numeric(substr(na.omit(dts$summer.start), 6,7))
mn_strt <- mean(as.Date(strptime(paste0(ifelse(mnths > 8, "2017","2018"),"-",substr(na.omit(as.character(dts$summer.start)),6,10)),format = "%Y-%m-%d")))

## BIGHORN - Statewide mean summer start ----
# May 28
# mn_strt <- as.Date("2017-03-05")

yrs <- dts$summr[is.na(dts$summer.start)==TRUE]

dts$summer.start[is.na(dts$summer.start)==TRUE] <- paste0(yrs, "-", substr(mn_strt,6,10), " 23:00:00")

mnths <- as.numeric(substr(na.omit(dts$summer.end), 6,7))
mn_end <- mean(as.Date(strptime(paste0(ifelse(mnths > 8, "2017","2018"),"-",substr(na.omit(as.character(dts$summer.end)),6,10)),format = "%Y-%m-%d")))

## BIGHORN - Statewide mean summer end ----
# Oct 29
# mn_end <- as.Date("2017-10-16")

yrs <- dts$summr[is.na(dts$summer.end)==TRUE]

dts$summer.end[is.na(dts$summer.end)==TRUE] <- paste0(yrs, "-", substr(mn_end,6,10), " 01:00:00")

dts$summer.start <- as.POSIXct(strptime(dts$summer.start,format = "%Y-%m-%d %H:%M:%S"), tz="GMT")
dts$summer.end <- as.POSIXct(strptime(dts$summer.end,format = "%Y-%m-%d %H:%M:%S"), tz="GMT")
...
for(i in 1:nrow(dts)){
  tmp <- d[d$date >= dts$summer.start[i] & d$date <= dts$summer.end[i] & d$id == dts$id[i],]
  ...
  if(nrow(tmp)>0){
    write.dbf(tmp, file = paste0(seqs_fldr,"/",sub("_","_su",dts$id_yr[i]),".dbf"))    #write as dbf
  }
  else{
    next
  }
...
for(i in 1:length(uBMvar)){
  ...
  fixInt <- ci$FixInterval_hrs[match(uids[i], ci$Animal_ID)]
...
stopCluster(cl)
write.csv(indBB, file=paste(metadata_fldr,"/metadata_summer.csv",sep=""), row.names=FALSE)

# ********************************
# 3. Population summer range ----
# ********************************
...
print(paste("You have ", length(fls), " sequences with successful Summer Range BBs.", sep=""))

ids <- str_split_fixed(fls, "_",3)[,1]

ids_unique <- unique(ids)
print(paste("You have ", length(ids_unique), " unique individuals with successful Summer Range BBs.", sep=""))

#average individual summer ranges over years
...
if(length(fls)==1){
  ...
  cs <- slot(slot(m, "grid"), "cellsize")
  slot(slot(m, "grid"), "cellsize") <- rep(mean(cs), 2)
  write.asciigrid(m, paste(pop_Rasters_out_fldr,"/","averageUD_summer.asc",sep=""), attr=1)
  projection(bb) <- out_proj
  writeRaster(bb, filename = paste(pop_Rasters_out_fldr,"/","averageUD_summer.img",sep=""), format="HFA")
  ...
  write.asciigrid(m, paste(pop_Rasters_out_fldr,"/","averageUD_summer.asc",sep=""), attr=1)
  projection(bb) <- out_proj
  writeRaster(bb, filename = paste(pop_Rasters_out_fldr,"/","averageUD_summer.img",sep=""), format="HFA") #change this fldr location to population loc for SR, WR, migs
  print(paste0("End time: ", Sys.time()))
}
